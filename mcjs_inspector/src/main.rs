...
fn main() {
    ...
    let app = relm4::RelmApp::new("mcjs.inspector");
    app.run::<RootState>(case_data);
}
...
enum VMState {
    ...
    Running {
        ...
        vm_thread: Option<JoinHandle<VMResult>>,
    },
    Suspended {
        vm_result: VMResult,
    },
...
enum RootMsg {
    Reset,
    Run(JoinHandle<VMResult>),
    Suspend(VMResult),
}
impl std::fmt::Debug for RootMsg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RootMsg::Reset => write!(f, "RootMsg::Reset"),
            RootMsg::Run(_) => write!(f, "RootMsg::Run"),
            RootMsg::Suspend(_) => write!(f, "RootMsg::Suspend"),
        }
    }
}

#[relm4::component]
impl relm4::SimpleComponent for RootState {
    type Input = RootMsg;
    type Output = ();
    type Init = CaseData;
    ...
    fn update(&mut self, msg: Self::Input, _sender: relm4::ComponentSender<Self>) {
        match msg {
            RootMsg::Reset | RootMsg::Run(_) | RootMsg::Suspend(_) => {}
        }
    }
}
